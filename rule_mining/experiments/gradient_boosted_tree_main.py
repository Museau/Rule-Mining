...
from rule_mining.models.gradient_boosted_tree import modelization
...
def main(file_name, prepro_type, nb_bin, binning_way):
    ...
    learning_rate = [1e2, 1e1, 1e0, 1e-1, 1e-2, 1e-3]
    ...
    path_gbt = 'results/' + file_name + '/gradient_boosted_tree/'

    if not os.path.exists(
            os.path.dirname(path_gbt)):
        os.makedirs(path_gbt)

    np.savez(
        path_gbt + 'scores_params',
        scores_params)
