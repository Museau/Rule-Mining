...
def modelization(x, y, cv, gbt_param, gscv):
    '''
    Gradient Boosted Tree using sklearn.
    ...
    - gbt_param, dict or dict of list. The dict correspond to Gradient Boosted
      Tree parameters from sklearn. If the dict is a dict of list. The lists
      contain the different parameters to test for the grid search cv.
    ...
    - feature_importances_, list (shape=[n_model]) of arrays,
      ...
      generator and n_features is the number of fatures.
    ...
    for train_index, test_index in cv.split(x, y):
        ...
        if gscv is True:
            # Do a grid search cv
            parameters = grid_search_cv(x_train, y_train, gbt_param)
            ...
            gbt = GradientBoostingClassifier(**gbt_param)
...
def modelization_gscv(x, y, cv, gbt_param):
    '''
    Gradient Boosted Tree model using sklearn.
    ...
    - y, pandas.Series, shape = [n_samples]
        Target vector relative to X.
    - cv, int, cross-validation generator.
    - gbt_param, dict.  The dict correspond to Gradient Boosted Tree
    ...
    feature_importances_, y_proba, score, best_param = modelization(
            x, y, cv, gbt_param, False)

    return score


def grid_search_cv(x, y, gbt_param):
    """
    ...
    - y, pandas.Series, shape = [n_samples]
        Target vector relative to X.
    - gbt_param, dict of list.  The dict correspond to Gradient Boosted Tree
      parameters from sklearn. The lists contain the different parameters to
      test for the grid search cv.

    Output:
    - parameters, dict. The dict correspond to Gradient Boosted Tree parameters
      choosen during the grid search cv.
    """

    product = [i for i in apply(itertools.product, gbt_param.values())]
    params = [dict(zip(gbt_param.keys(), p)) for p in product]
